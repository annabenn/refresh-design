...
if (isset($_POST['login-submit'])) {
  ...
  if (empty($mailuid) || empty($password)) {
    ...
    if(strcmp($userType,"Client")==0){
        ...
        $sql = "SELECT * FROM designers WHERE uidDesigners=? OR emailDesigners=?";
    }
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($row = mysqli_fetch_assoc($result)) {
        ...
        if(strcmp($userType,"Client")==0){
          ...
          $pwdCheck = password_verify($password, $row['pwdDesigners']);
        }
        // If they don't match thenwe create an error message!
        //scriu aici
        ...
        // Then if they DO match, then we know it is the correct user that is trying to log in!
          ...
          if(strcmp($userType,"Client")==0){
              ...
              $_SESSION['id'] = $row['idDesigners'];
              $_SESSION['uid'] = $row['uidDesigners'];
              $_SESSION['email'] = $row['emailDesigners'];
